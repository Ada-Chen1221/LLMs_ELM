#!/usr/bin/env python
"""QLoRA SFT training with 4-bit BitsAndBytes + PEFT + TRL."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_dir", default="outputs/qwen3_1p7b_qlora")
    parser.add_argument(
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(
            "Missing QLoRA dependency. Install with: pip install -r requirements.txt "
            "and make sure bitsandbytes is available for your CUDA/PyTorch version."
        ) from exc
    ...
    if not torch.cuda.is_available():
        print("Warning: CUDA is not available. bitsandbytes 4-bit QLoRA normally requires a CUDA GPU.")
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map={"": int(__import__("os").environ.get("LOCAL_RANK", "0"))} if torch.cuda.is_available() else None,
            trust_remote_code=True,
        )
    except Exception as exc:
        raise SystemExit(
            f"Failed to load 4-bit model/tokenizer '{args.model_name_or_path}'. "
            "Check CUDA, bitsandbytes, model name/path, HuggingFace permissions, and local cache. "
    ...
    if args.adapter_path:
        print(f"Loading existing QLoRA adapter for continued training: {args.adapter_path}", flush=True)
