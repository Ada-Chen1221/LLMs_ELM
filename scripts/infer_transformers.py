...
def build_prompt_text(tokenizer, prompt: str, system_prompt: str | None) -> str:
    ...
    This follows the Qwen official quickstart style: build `messages`, call
    `apply_chat_template(..., tokenize=False, add_generation_prompt=True)`, then
    tokenize the rendered text in one normal tokenizer call.
    """
...
def first_model_device(model):
    """Return the first usable model device for placing input tensors."""
...
def main() -> None:
    ...
    model.eval()
    log_step("[3/5] Rendering chat template and tokenizing prompt...")
